...
res_to_use <- bind_rows(res_list[[1]] %>% filter(.id == 545) %>% mutate(season = seasons[1]),
                        res_list[[2]] %>% filter(.id == 596) %>% mutate(season = seasons[2]),
                        res_list[[3]] %>% filter(.id == 812) %>% mutate(season = seasons[3]),
                        res_list[[4]] %>% filter(.id == 108) %>% mutate(season = seasons[4]),
                        res_list[[5]] %>% filter(.id == 254) %>% mutate(season = seasons[5]),
