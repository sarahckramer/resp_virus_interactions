...
for (i in 1:length(seasons)) {
  ...
  res_temp <- run_sim(po_list[[i]], seasons[i], mle_us, shared_estpars_us, unit_estpars, model_type = 'deterministic', return_obs = TRUE, analysis = 'basic')
...
ar_us <- bind_rows(ar_list) %>%
  mutate(loc = 'us')

# Clean up:
rm(i, dat_pomp, hk_dat, can_dat, us_dat, obj_fun_list, po_list, res_temp,
   ar_temp, debug_bool, vir1, vir2, Ri_max1, Ri_max2, d2_max, prof_lik,
   fit_canada, fit_us, region, age_structured, yr, yr_index, nrow_check)

# Save results for supplementary figure:
ar_df <- bind_rows(ar_hk, ar_can, ar_us) %>%
...
ar_df %>% filter(loc == 'us' & virus == 'Influenza') %>% pull(attack_rate) %>% summary() %>% print()
ar_df %>% filter(loc == 'us' & virus == 'RSV') %>% pull(attack_rate) %>% summary() %>% print()
