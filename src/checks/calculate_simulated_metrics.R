...
# Simulate several outbreaks for each season (US):
fit_canada <- FALSE
fit_us <- TRUE
vir1 <- 'flu'
...
for (i in 1:length(seasons)) {
  ...
  # Run simulations:
  sim_temp <- run_sim(po_list[[i]], seasons[i], mle_us, shared_estpars_us, unit_estpars, model_type = 'stochastic', n_sim = 100)
  
  p_temp <- ggplot() +
    geom_line(data = sim_temp, aes(x = time, y = n_P2, group = .id), col = 'gray80', alpha = 0.5) +
    geom_point(data = us_dat %>% filter(season == seasons[i]), aes(x = time, y = n_P2)) +
    theme_classic()
  print(p_temp)
  ...
  # Calculate metrics (observed):
  obs_metrics <- calculate_metrics(us_dat %>% filter(season == seasons[i]))
  
  # Also calculate correlation coefficients for full outbreaks:
  sim_corr <- sim_temp %>%
    full_join(us_dat %>% filter(season == seasons[i]),
              by = 'time') %>%
    group_by(.id) %>%
    summarise(corr1 = cor(n_P1.x, n_P1.y, use = 'pairwise.complete.obs'),
              corr2 = cor(n_P2.x, n_P2.y, use = 'pairwise.complete.obs'))
