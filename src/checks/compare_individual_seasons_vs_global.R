...
res_files <- list.files(path = 'results/160921_fluA_thetalambda1_round1ci_fitR_rho2=15/', full.names = TRUE)
...
# Get list of results files:
res_files <- list.files(path = 'results/170921_fluA_thetalambda1_round1CI_fixRi2I20/', full.names = TRUE)
...
res_glob <- pars_top %>%
  ...
  mutate(year = str_sub(param, 1, 4),
         param = str_sub(param, 6, str_length(param))) %>%
  select(param:year) %>%
  mutate(method = 'Global_CI_fixRi2I20')
...
# Get list of results files:
res_files <- list.files(path = 'results/240921_fluA_thetalambda1_round1CI_fixRi2I20_early/', full.names = TRUE)
...
res_glob <- pars_top %>%
  ...
  mutate(year = str_sub(param, 1, 4),
         param = str_sub(param, 6, str_length(param))) %>%
  select(param:year) %>%
  mutate(method = 'Global_CI_fixRi2I20_early')
...
# Get list of results files:
res_files <- list.files(path = 'results/240921_fluA_thetalambda1_round1CI_fixRi2I20_early_rho2_delta/', full.names = TRUE)
...
pars_top <- pars_df[1:no_best, ]
# Interestingly, ll is very different here - only top result is "best" fit
...
# Compare:
res_glob <- pars_top %>%
  pivot_longer(-c(theta_lambda1, delta, rho2, loglik), names_to = 'param') %>%
  mutate(year = str_sub(param, 1, 4),
         param = str_sub(param, 6, str_length(param))) %>%
  select(param:year) %>%
  mutate(method = 'Global_CI_fixRi2I20_early_rho2delta')
