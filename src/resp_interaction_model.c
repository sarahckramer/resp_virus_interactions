...
T_theta_lambda_I1 = logit(theta_lambda_I1);
T_theta_lambda_I2 = logit(theta_lambda_I2);
T_theta_lambda_T1 = logit(theta_lambda_T1);
T_theta_lambda_T2 = logit(theta_lambda_T2);
...
theta_lambda_I1 = expit(T_theta_lambda_I1);
theta_lambda_I2 = expit(T_theta_lambda_I2);
theta_lambda_T1 = expit(T_theta_lambda_T1);
theta_lambda_T2 = expit(T_theta_lambda_T2);
...
DX_IS = lambda1 * X_SS - (gamma1 + theta_lambda_I1 * lambda2) * X_IS; 
DX_TS = gamma1 * X_IS - (delta1 + theta_lambda_T1 * lambda2) * X_TS;
DX_RS = delta1 * X_TS - lambda2 * X_RS; 
DX_SI = lambda2 * X_SS - (theta_lambda_I2 * lambda1 + gamma2) * X_SI;
DX_II = theta_lambda_I1 * lambda2 * X_IS + theta_lambda_I2 * lambda1 * X_SI - (gamma1 + gamma2) * X_II; 
DX_TI = theta_lambda_T1 * lambda2 * X_TS + gamma1 * X_II - (delta1 + gamma2) * X_TI;
DX_RI = lambda2 * X_RS + delta1 * X_TI - gamma2 * X_RI; 
DX_ST = gamma2 * X_SI - (theta_lambda_T2 * lambda1 + delta2) * X_ST; 
DX_IT = gamma2 * X_II + theta_lambda_T2 * lambda1 * X_ST - (gamma1 + delta2) * X_IT; 
...
rates[1] = lambda2;
rates[2] = gamma1;
rates[3] = theta_lambda_I1 * lambda2;
rates[4] = delta1;
rates[5] = theta_lambda_T1 * lambda2;
rates[6] = theta_lambda_I2 * lambda1;
rates[7] = gamma2;
rates[8] = gamma1;
rates[9] = gamma2;
rates[10] = delta1;
rates[11] = gamma2;
rates[12] = theta_lambda_T2 * lambda1;
rates[13] = delta2;
rates[14] = gamma1;
rates[15] = delta2;
rates[16] = delta1;
