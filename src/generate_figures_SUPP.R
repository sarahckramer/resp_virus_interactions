...
# Supplementary Figure 1: Number of total tests for respiratory viruses over time
...
fig2s <- ggplot(data = dat_pos, aes(x = Time, y = perc_pos, col = virus)) +
...
fig2s <- reposition_legend(fig2s, position = 'top left', plot = FALSE)

ggsave('results/plots/figures_for_manuscript/supp/FigureS2.svg', width = 9.5, height = 4, fig2s)

rm(dat_pos, fig1s, fig2s)
...
rm(dat_hk, dat_clim, p3a, p3b, p3c, fig3s, x_lab_breaks)
...
p4a <- ggplot(data = res %>% filter(vir1 == 'flu_h1')) +
...
p4b <- ggplot(data = res %>% filter(vir1 == 'flu_b')) +
  geom_errorbar(aes(x = season, ymin = lower, ymax = upper)) +
  geom_point(aes(x = season, y = mle)) +
  facet_wrap(~ parameter, scales = 'free_y', nrow = 1) +
  theme_classic() +
...
ggsave('results/plots/figures_for_manuscript/supp/FigureS4.svg', width = 26, height = 8.4, fig4s)

rm(p4a, p4b, fig4s, res, mle, cis)

# ---------------------------------------------------------------------------------------------------------------------

# Supplementary Figure 5: Correlations between estimated parameters
...
load_and_format_mega_results <- function(filepath, shared_estpars, unit_estpars, run_name) {
  ...
  return(list(pars_top, pars_top_LONG))
...
pars_top_LIST = pars_top_long_LIST = vector('list', length = length(res_LIST))
...
fig5sa <- ggpairs(pars_top_LIST_temp[[1]] %>% select(all_of(shared_estpars_temp)),
...
fig5sb <- ggpairs(pars_top_LIST_temp[[2]] %>% select(all_of(shared_estpars_temp)),
                  upper = list(continuous = wrap(ggally_cor, size = 4.5, method = 'kendall', digits = 2, display_grid = FALSE)),
                  labeller = 'label_parsed') +
  theme_classic() +
  theme(axis.text = element_text(size = 11),
...
ggsave('results/plots/figures_for_manuscript/supp/FigureS5a.svg', fig5sa, width = 18, height = 12)
ggsave('results/plots/figures_for_manuscript/supp/FigureS5b.svg', fig5sb, width = 18, height = 12)

rm(fig5sa, fig5sb, pars_top_LIST_temp, shared_estpars_temp)

# ---------------------------------------------------------------------------------------------------------------------

# Supplementary Figure 6: Beta due to climate over time
...
p6a <- ggplot(data = beta_h1, aes(x = time, y = beta1, col = season)) +
  geom_line() +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
p6b <- ggplot(data = beta_h1, aes(x = time, y = beta2, col = season)) +
  geom_line() +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
p6c <- ggplot(data = beta_b, aes(x = time, y = beta1, col = season)) +
  geom_line() +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
p6d <- ggplot(data = beta_b, aes(x = time, y = beta2, col = season)) +
  geom_line() +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
fig6s <- arrangeGrob(arrangeGrob(p6a, p6b, p6c, p6d, ncol = 2), p_legend, nrow = 2, heights = c(15, 1))
ggsave('results/plots/figures_for_manuscript/supp/FigureS6.svg', fig6s, width = 15, height = 8.5)

rm(mles, beta_h1, beta_b, beta_temp, hk_dat, dat_clim, p6a, p6b, p6c, p6d, p_legend, fig6s,
...
fig7s <- arrangeGrob(p7a, p7b, ncol = 1)
ggsave('results/plots/figures_for_manuscript/supp/FigureS7.svg', width = 21, height = 7.5, fig7s)

rm(fig7s, p7a, p7b, sim_h1, sim_b)
...
ggsave('results/plots/figures_for_manuscript/supp/FigureS8.svg', width = 11.5, height = 4.2, fig8s)

rm(fig8s, p8a, p8b, res_proflik, maxloglik, ci_cutoff)
...
p9a <- ggplot(data = ar_df %>% filter(virus1 == 'H1'),
...
p9b <- ggplot(data = ar_df %>% filter(virus1 == 'B'),
              aes(x = virus, y = attack_rate, group = virus)) +
  geom_violin(fill = 'gray90') +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
...
ggsave('results/plots/figures_for_manuscript/supp/FigureS9.svg', width = 10, height = 4, fig9s)

rm(fig9s, p9a, p9b, ar_df)
...
fig10s <- arrangeGrob(p10a, p10b, ncol = 1, heights = c(1.6, 1))
ggsave('results/plots/figures_for_manuscript/supp/FigureS10.svg', width = 16, height = 9.5, fig10s)

rm(p10a, p10b, fig10s, res)
...
# Supplementary Figure 12: Impact of LAIV by season for both scenarios
